...
export async function fetchStartOptions() {
  const r = await fetch(`${API}/start-options`);
  if (!r.ok) throw new Error('Failed to load start options');
  return r.json();
}

export async function fetchNeighbors(nodeName, radius) {
...
export async function fetchExplanation(pathId) {
  const r = await fetch(`${API}/explain?pathId=${encodeURIComponent(pathId)}`);
  if (!r.ok) throw new Error('Explanation not found');
  return r.json();
}

export async function runSimulation(scenarioId, mutations, analysis) {
